...
public class CGenerator implements CodeGenerator
{
        ...
        final String structName, final String outermostStruct, final List<Token> tokens)
    {
        ...
        for (int i = 0, size = tokens.size(); i < size;)
        {
            ...
            sb.append(String.format("\n" +
                "SBE_ONE_DEF uint64_t %5$s_%1$s_skip(\n" +
                "    %5$s *const codec)\n" +
                "{\n" +
                "    uint64_t ret = 0;\n" +
                "%2$s" +
            ...
            sb.append(String.format("\n" +
                ...
                identBlock(
                "    uint64_t length_position = %5$s_sbe_position(codec);\n" +
                "    %3$s length_value = %4$s((%3$s)length);\n" +
                "    if (%5$s_set_sbe_position(codec, length_position + sizeof(length_value) + length))\n" +
                "    {\n" +
                "        memcpy(codec->buffer + length_position, &length_value, sizeof(length_value));\n" +
                "        memcpy(codec->buffer + length_position + sizeof(length_value), src, (size_t)length);\n\n" +
                "        return codec;\n" +
                "    }\n") +
                "    return NULL;\n" +
                "}\n\n" +
        ...
        final String containingStructName, final String outermostStruct, final String propertyName, final Token token)
    {
        ...
        {
            return generateArrayProperty(containingStructName, outermostStruct, propertyName, token, true);
        }
        ...
        final String propertyName, final Token token, final String containingStructName)
    {
        ...
        sb.append(String.format("\n" +
            "SBE_ONE_DEF sbe_const_string_view %3$s_%1$s(void)\n" +
            "{\n" +
            "    static const uint8_t %3$s_%1$s_values[] = {%2$s};\n" +
            "    sbe_const_string_view view;\n" +
            "    view.data = (const char *)(%3$s_%1$s_values);\n" +
            "    view.length = %4$d;\n" +
            "    return view;\n" +
            "}\n",
            propertyName,
        ...
        final StringBuilder sb, final Token token, final String containingStructName, final String outermostStruct)
    {
        ...
        sb.append(String.format("\n" +
            ...
            "    const sbe_meta_attribute attribute)\n" +
            "{\n" +
            "    switch (attribute)\n" +
            "    {\n" +
        ...
        final Token token)
    {
        ...
        else
        {
            ...
            if (arrayCapacity >= 0)
            {
                sb.append(generateArrayProperty(containingStructName, outermostStruct, propertyName, token, false));
            }
            sb.append(String.format("\n" +
                "SBE_ONE_DEF bool %7$s_%2$s_safe(\n" +
